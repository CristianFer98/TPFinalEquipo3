...
public class ControladorAmbulanciaTest{
	...
	public void adminPuedeMarcarSolicitudComoAtendida() throws ParseException {
		...
		ModelAndView mav= controladorAmbulancia.darDeAltaSolicitud("test@gmail.com", mockedRequest);
		
		assertThat(mav.getModel().get("msj")).isEqualTo("Estado consulta cambiado con exito!");
		assertThat(mav.getViewName()).isEqualTo("ambulanciaControl");	
		
	}
	
	@Test
	public void testQueMePermitaCancelarUnaAmbulancia() {
		...
		when(servicioAmbulancia.obtenerAmbulanciaPorPatente(patente)).thenReturn(ambulancia);
		when(servicioRegistroLogin.obtenerUsuarioPorId(1)).thenReturn(new Usuario());
		
		mav = controladorAmbulancia.cancelarAmbulancia(patente, mockedRequest);
		
		assertThat(mav.getViewName()).isEqualTo("cancelacionAmbulancia");
